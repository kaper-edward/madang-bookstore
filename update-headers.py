#!/usr/bin/env python3
"""
모든 HTML 파일의 헤더와 footer를 통일하는 스크립트
"""
import re
import os

# frontend 디렉토리의 HTML 파일 목록
html_files = [
    'frontend/books.html',
...
def update_html_file(filepath, current_page=''):
    ...
    if 'header.js' not in content:
        # api.js 다음에 header.js 추가
        content = content.replace(
            '<script src="js/api.js"></script>',
            '<script src="js/api.js"></script>\n  <script src="js/header.js"></script>'
        )

    # 4. renderHeader()와 renderFooter() 호출 추가
    # DOMContentLoaded 또는 window.addEventListener 찾기
    # 파일마다 초기화 방식이 다르므로, 각 파일을 수동으로 처리해야 함
    # 대신 일반적인 패턴으로 추가
...
print("\n✅ All files updated!")
print("⚠️  Note: You need to manually add renderHeader() and renderFooter() calls to each page's initialization code.")
